...
// reCAPTCHA is optional — bypassed if VITE_RECAPTCHA_SITE_KEY is not set
...
const ContactForm = () => {
  ...
  const [captchaDone, setCaptchaDone] = useState(!RECAPTCHA_SITE_KEY); // true if no key
  ...
  const validateField = (k, v) => {
    ...
    if (k === 'email')
      return v.trim() === ''
        ? 'contact.err_email_required'
        : !EMAIL_REGEX.test(v.trim())
          ? 'contact.err_email_invalid'
          : '';
    if (k === 'message')
      return v.trim() === '' ? 'contact.err_message_required' : '';
    return '';
  };
    ...
    <Styled.ContactSection id="contact">
      <Styled.ContactInner>
        ...
        <Styled.ContactForm onSubmit={handleSubmit} noValidate>
          ...
          {/* ── reCAPTCHA v2 — only rendered when VITE_RECAPTCHA_SITE_KEY is set ── */}
          {RECAPTCHA_SITE_KEY && (
          ...
          <AnimatePresence mode="wait">
            {status && status !== 'sending' && (
              ...
              >
                {t(
                  `contact.${status === 'ok' ? 'success' : status === 'cooldown' ? 'cooldown' : 'error'}`
                )}
              </Styled.ContactStatus>
            )}
          </AnimatePresence>
          ...
          <Styled.ContactSubmitBtn
            ...
            whileTap={
              !isSending && !disabled && captchaDone ? { scale: 0.97 } : {}
            }
            whileHover={
              !isSending && !disabled && captchaDone ? { opacity: 0.85 } : {}
            }
          >
